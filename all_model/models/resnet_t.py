...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.discriminator = nn.Sequential(  # Resnet50  2048 1024
            nn.Linear(2048, 1024),
    ...
    def forward(self, x, is_feat=False, feat_s=None, feat_t=None):
        if not feat_s is None:
            ...
            x = x.view(x.size(0), -1)
            f5 = x
            out = self.discriminator(GradReverse(self.lambd).forward(x))
            x = self.fc(x)

            if is_feat:
                return [f0, f1, f2, f3, f4, f5], x, out
            else:
                return x, out
